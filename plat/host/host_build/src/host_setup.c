					 ...
					 unsigned long *donate_req)
{
	...
	while (rc.status == RMI_INCOMPLETE) {
		...
		case RMI_OP_MEM_REQ_DONATE: {
			unsigned long count = EXTRACT(RMI_COUNT, *donate_req);
			...
			assert(EXTRACT(RMI_ADDR_BLK_SIZE, *donate_req) == RMI_PAGE_L3);
...
static int rtt_data_unmap_range(void *rd, uintptr_t base_ipa, uintptr_t top_ipa)
{
	...
	while (current_ipa < top_ipa) {
		...
		/*
		 * @TODO: Disable this assertion for now as it is triggering
		 * false negatives.
		 *
		 * Revisit it once RMM can unmap a range properly.
		 */
...
static int host_destroy_realm(struct host_realm *realm)
{
	...
	/* @TODO: At the moment, unmap_range only supports one granule at a time */
	for (i = 0U; i < realm->realm_data_1_num_gr; i++) {
		uintptr_t base_ipa = REALM_BUFFER_IPA_1 + (i * GRANULE_SIZE);
		uintptr_t top_ipa = base_ipa + GRANULE_SIZE;

		if (rtt_data_unmap_range(realm->rd, base_ipa, top_ipa) != 0) {
			return -1;
		}
	}
