	...
	uint64_t top)
{
	...
	__tb_regs.X0 = SMC_RMI_GRANULE_RANGE_DELEGATE;
	...
	bool failure_gran_state_pre = Granule(base).state != UNDELEGATED;
	bool failure_gran_gpt_pre = Granule(base).gpt != GPT_NS;
	...
	bool failure_gran_state_post = ResultEqual_2(result, RMI_ERROR_INPUT) &&
					       (next_addr == base);
	bool failure_gran_gpt_post = ResultEqual_2(result, RMI_ERROR_INPUT) && (next_addr == base);
	bool success_gran_state_post = Granule(base).state == DELEGATED;
	bool success_gran_gpt_post = Granule(base).gpt == GPT_REALM;
