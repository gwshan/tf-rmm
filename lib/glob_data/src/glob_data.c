		...
		unsigned long max_gr, unsigned long max_dev_gr)
{
	...
	/* Calculate the total number of SRO contexts needed */
	max_sro_ctxs = rmm_el3_ifc_get_total_cpus() * SRO_CTX_PER_CPU;

	/*
	 * Allocate space to store the array of contexts along with the
	 * sro_state array and the per_cpu array.
	 *
	 * @TODO: Need to double check that there will be enough space,
	 *        as some padding might occur.
	 */
	new_gl->sro_ctxs_sz = (sizeof(struct sro_context) + sizeof(enum sro_state)) *
									max_sro_ctxs;
