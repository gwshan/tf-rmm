...
static bool __unused rsi_vdev_matches_map(struct rd_aux *rd_aux, unsigned long vdev_id,
				 struct granule *g_vdev)
{
 ...
 * addresses are provided by the caller and RMM locks them in sorted order.
 *
					    ...
					    struct rsi_vdev_obj *lock_set, bool claim_pdev)
{
	...
	*lock_set = (struct rsi_vdev_obj){0};
	...
	rd_aux = buffer_rd_aux_granules_map(
		&lock_set->rd->aux_granules[0], lock_set->rd->num_rd_aux);
	assert(rd_aux != NULL);
	assert(rsi_vdev_matches_map(rd_aux, vdev_id, lock_set->g_vdev));
	buffer_rd_aux_granules_unmap(rd_aux, lock_set->rd->num_rd_aux);
