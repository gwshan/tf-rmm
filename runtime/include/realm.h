...
/*
 * Reads the packed state_and_count with acquire semantics (lock-free).
 */
static inline unsigned long rd_read_state_count(struct rd *rd)
{
	return SCA_READ64_ACQUIRE(&rd->state_and_count);
}

/*
 * Gets the realm state from a pre-read packed value.
 */
static inline unsigned long rd_get_state(unsigned long packed)
{
	return RD_UNPACK_STATE(packed);
}
