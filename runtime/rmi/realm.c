...
{
	assert(realm_ctx != NULL);

	revert_sl_rtts(realm_ctx->rtt_base,
		       realm_ctx->rtt_num_start,
		       realm_ctx->n_rtts);
	free_vmids(realm_ctx->vmid, realm_ctx->n_vmids);
...
static void realm_sro_reclaim_finish(unsigned long fid, struct smc_result *res)
{
		      ...
		      struct smc_result *res)
{
	...
	realm_ctx->flags0 = p.flags0;
	realm_ctx->flags1 = p.flags1;
	realm_ctx->s2sz = p.s2sz;
	realm_ctx->sve_vl = p.sve_vl;
	realm_ctx->num_bps = p.num_bps;
	realm_ctx->num_wps = p.num_wps;
	realm_ctx->pmu_num_ctrs = p.pmu_num_ctrs;
	realm_ctx->num_aux_planes = p.num_aux_planes;
	realm_ctx->rtt_num_start = p.rtt_num_start;
	realm_ctx->rtt_level_start = p.rtt_level_start;
	realm_ctx->ats_plane = ats_plane;
	realm_ctx->n_vmids = n_vmids;
	realm_ctx->n_rtts = n_rtts;
	realm_ctx->mecid = mecid;
	realm_ctx->num_rd_aux = num_rd_aux;
	realm_ctx->rtt_tree_pp = rtt_tree_pp;
	realm_ctx->algorithm = p.algorithm;
	(void)memcpy(realm_ctx->rpv, p.rpv, sizeof(realm_ctx->rpv));
	(void)memcpy(realm_ctx->realm_instance_id, realm_instance_id,
		     sizeof(realm_ctx->realm_instance_id));
	(void)memcpy(realm_ctx->vmid, vmid, sizeof(realm_ctx->vmid));
	(void)memcpy(realm_ctx->rtt_base, rtt_base, sizeof(realm_ctx->rtt_base));
...
static void realm_create_continue(unsigned long fid, struct smc_result *res)
{
	...
	struct rd *rd;
	struct sro_realm_ctx *realm_ctx;
	struct rmi_realm_params realm_params = {0};
	...
	if (sro->aux_op_ctx.total_transferred != realm_ctx->num_rd_aux) {
		ret = RMI_ERROR_INPUT;
		goto out_unlock;
	}

	for (unsigned int i = 0U; i < realm_ctx->num_rd_aux; i++) {
	...
	rd->rtt_tree_pp = realm_ctx->rtt_tree_pp;
	rd->num_aux_planes = realm_ctx->num_aux_planes;
	rd->rtt_s2ap_encoding = (EXTRACT(RMI_REALM_FLAGS1_S2AP_ENC,
					 realm_ctx->flags1) != 0UL);
	...
	for (unsigned int idx = 0U; idx < realm_num_s2_contexts(rd); idx++) {
		...
		s2tt_ctx->g_rtt = find_granule(rd->rtt_tree_pp ?
					       realm_ctx->rtt_base[idx] :
					       realm_ctx->rtt_base[0]);
		s2tt_ctx->ipa_bits = realm_ctx->s2sz;
		s2tt_ctx->s2_starting_level = (int)realm_ctx->rtt_level_start;
		s2tt_ctx->num_root_rtts = realm_ctx->rtt_num_start;
		s2tt_ctx->enable_lpa2 =
			(EXTRACT(RMI_REALM_FLAGS0_LPA2, realm_ctx->flags0) == RMI_FEATURE_TRUE);
		s2tt_ctx->indirect_s2ap = rd->rtt_s2ap_encoding;
		s2tt_ctx->mecid = realm_ctx->mecid;
	}
	...
	for (unsigned int plane_idx = 0U; plane_idx < realm_num_planes(rd); plane_idx++) {
		...
		s2tt_ctx->vmid = realm_ctx->vmid[plane_idx];
	}

	(void)memcpy(&rd->rpv[0], &realm_ctx->rpv[0], RPV_SIZE);
	(void)memcpy(rd->realm_instance_id, realm_ctx->realm_instance_id,
		     sizeof(rd->realm_instance_id));

	rd->num_rec_aux = MAX_REC_AUX_GRANULES;
	rd->num_rd_aux = realm_ctx->num_rd_aux;

	for (unsigned int i = 0U; i < rd->num_rd_aux; i++) {
		rd->aux_granules[i] = g_aux_list[i];
	}

	rd->simd_cfg.sve_en = EXTRACT(RMI_REALM_FLAGS0_SVE, realm_ctx->flags0) != 0UL;
	if (rd->simd_cfg.sve_en) {
		rd->simd_cfg.sve_vq = realm_ctx->sve_vl;
	}

	if (realm_ctx->algorithm == RMI_HASH_SHA_256) {
		rd->algorithm = HASH_SHA_256;
	} else if (realm_ctx->algorithm == RMI_HASH_SHA_512) {
		rd->algorithm = HASH_SHA_512;
	} else {
		rd->algorithm = HASH_SHA_384;
	}

	rd->pmu_enabled = EXTRACT(RMI_REALM_FLAGS0_PMU, realm_ctx->flags0) != 0UL;
	rd->pmu_num_ctrs = realm_ctx->pmu_num_ctrs;

	rd->ats_plane = realm_ctx->ats_plane;
	...
	realm_params.flags0 = realm_ctx->flags0;
	realm_params.s2sz = realm_ctx->s2sz;
	realm_params.sve_vl = realm_ctx->sve_vl;
	realm_params.num_bps = realm_ctx->num_bps;
	realm_params.num_wps = realm_ctx->num_wps;
	realm_params.pmu_num_ctrs = realm_ctx->pmu_num_ctrs;
	realm_params.algorithm = realm_ctx->algorithm;
...
void realm_continue_handler(unsigned long fid, struct smc_result *res)
{
	const sro_handle_cb sro_callbacks[] = {
		...
		[SRO_OBJ_DESTROY_FINISH] = realm_sro_reclaim_finish
	};
	struct sro_context *sro = my_sro_ctx();
