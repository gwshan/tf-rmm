...
void smc_rec_destroy(unsigned long rec_addr, struct smc_result *res)
{
	...
	/*
	 * REC is now unlocked, but it remains in GRANULE_STATE_PARTIAL.
	 * Therefore no RMI command, including REC_ENTER, will treat it as a
	 * live REC or modify it.
	 *
	 * The RD reference held by this REC still keeps g_rd valid, and the
	 * MPIDR-to-REC association cannot change underneath us: replacing or
	 * deleting it would require destroying this REC, which is no longer
	 * possible once it has left GRANULE_STATE_REC, and rd->rec_count is
	 * never decremented.
	 *
	 * It is therefore safe to remove the MPIDR mapping while holding only
	 * the RD lock; the REC lock is no longer needed.
	 */
