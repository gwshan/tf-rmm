...
static struct granule *init_fake_rd(uintptr_t rd_pa, uintptr_t rd_aux_pa_out[])
{
	...
	for (unsigned int i = 0U; i < MAX_RD_AUX_GRANULES; i++) {
		rd_aux_pa_out[i] = test_helpers_allocate_granules(1U);
		CHECK_TRUE(delegate_range(rd_aux_pa_out[i],
					  rd_aux_pa_out[i] + GRANULE_SIZE));

		struct granule *g_rd_aux = find_granule(rd_aux_pa_out[i]);
	...
	for (unsigned int i = 0U; i < rd->num_rd_aux; i++) {
		rd->aux_granules[i] = find_granule(rd_aux_pa_out[i]);
	}
...
static void add_fake_rec_mapping(struct granule *g_rd,
				 unsigned long mpidr,
				 uintptr_t rec_pa)
{
	struct rd *rd;
				...
				uintptr_t aux_pa_out[])
{
	...
	struct granule *g_rd = init_fake_rd(rd_pa, rd_aux_pa);
	...
	add_fake_rec_mapping(g_rd, mpidr, rec_pa);
